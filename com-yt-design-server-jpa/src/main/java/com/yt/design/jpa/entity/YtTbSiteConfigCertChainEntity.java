...
@DynamicUpdate
@Transactional
@Table(name = "yt_tb_site_config_cert_chain", schema = "yitaodesign", catalog = "")

public class YtTbSiteConfigCertChainEntity implements DbDataCommonDataEntityChecker {
    ...
    @Column(name = "certChainName")
    private String certChainName;//证书模板名称

    @Column(name = "certChainDesc")
    private String certChainDesc;//证书模板描述

    @Column(name = "certChainTempleUrl")
    private String certChainTempleUrl;//证书模板图片URL
    ...
    public boolean dataCheckForCreate() throws DataCheckCommonException {
        if (this.certChainName == null || this.certChainName.length() <= 0) {
            throw new DataCheckCommonException("证书模板不能为空");
        }
        if (this.certChainTempleUrl == null || this.certChainTempleUrl.length() <= 0) {
            throw new DataCheckCommonException("证书模板图片不能为空");
        }
